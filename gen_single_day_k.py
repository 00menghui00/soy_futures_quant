...
# 读取 CSV 数据
EUA = pd.read_csv(r"F:\Futures_Quant\preprocess\全-数值型-大豆、豆粕、豆油\豆油.csv", encoding='gbk')

# 确保日期列是 datetime 格式
EUA['date'] = pd.to_datetime(EUA['date'])

# 创建输出文件夹（如果不存在的话）
output_dir = r"F:\Futures_Quant\preprocess\gen_single_day_K_photo\soy_oil_pic"
os.makedirs(output_dir, exist_ok=True)

# 定义绘制 K 线图的函数
def generate_k_line_chart(i):
    if i < len(EUA):  # 确保我们不会超出数据的索引范围
        seg_EUA = EUA.iloc[i:i+1]  
        ...
        fig.savefig(
            os.path.join(output_dir, f"{seg_EUA.index[0].strftime('%Y-%m-%d')}_single_day_k_line_no_grid.jpg"),
